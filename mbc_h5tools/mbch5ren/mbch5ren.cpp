...
{
    std::cerr
        << "Renames an instrument in h5 file. The path to the instrument within the file remains the same." << std::endl
        << "Arguments: file.h5:/path/oldName newName" << std::endl;
    return -1;
}
...
/*
            ...
            try
            {
                using (Repository repository = Repository.OpenReadWrite(file, false))
                {
                    ...
                    repository.RenameInstrument(path, args[1]);
                }
            }
            catch (Exception ex)
            {
