...
std::string to_string(const TradePriceOnly& trade)
{
    std::ostringstream oss;
    oss << "t:" << trade.ticks_ << ", p:" << trade.price_;
    return oss.str();
}

TESTGROUP("h5 exploratory trade price-only data")
{
    TESTCASE_WITH_CONTEXT("data count, first ticks, last ticks")
    {
        const char* fileName{ "exploratory_trade_price-only_dataCount_firstTicks_lastTicks.h5" };
        remove(fileName);
        ...
        ASSERT_NOT_NULL(data) << step << ": instrument->openTradePriceOnlyData should return non-null data";

        step = "empty data";
        verifyProperties(data, 0, 0, 0, step, context);

        step = "add initial data";
        std::vector<TradePriceOnly> trades
        {
            { 87, 0.87 },
            { 88, 0.88 },
            { 89, 0.89 },
        ...
        add(trades, data, DuplicateTimeTicks::Fail, false, true, step, context);
        verifyFetchAll(trades.data(), trades.size(), data, step, context);

        step = "filled data";
        verifyProperties(data, trades.size(), trades.front().ticks_, trades.back().ticks_, step, context);
    ...
    {
        const char* fileName{ "exploratory_trade_price-only_add_update_prepend_append_insert.h5" };
        remove(fileName);
        ...
        add(trades, data, DuplicateTimeTicks::Update, false, true, step, context);
        verifyFetchAll(trades.data(), trades.size(), data, step, context);

        step = "update first, DuplicateTimeTicks::Update";
        std::vector<TradePriceOnly> update1
        {
            {100, 1.01}
        };
        ...
        std::vector<TradePriceOnly> update2
        {
            { 100, 1.0 },
            { 130, 1.31 }
        };
        ...
        std::vector<TradePriceOnly> update3
        {
            { 130, 1.3 },
            { 150, 1.51 }
        };
        ...
        std::vector<TradePriceOnly> update4
        {
            { 150, 1.5 }
        };
        ...
        std::vector<TradePriceOnly> update5
        {
            { 97, 0.97 },
            { 98, 0.98 },
            { 99, 0.99 }
        ...
        trades.insert(trades.begin(), update5.begin(), update5.end());
        verifyFetchAll(trades.data(), trades.size(), data, step, context);

        step = "prepend equal ticks, DuplicateTimeTicks::Fail";
        std::vector<TradePriceOnly> update6
        {
            { 94, 0.94 },
            { 94, 0.95 },
            { 94, 0.96 }
        ...
        trades.insert(trades.begin(), update6.begin(), update6.end());
        trades[1].ticks_ = 95;
        trades[2].ticks_ = 96;
        verifyFetchAll(trades.data(), trades.size(), data, step, context);

        step = "prepend equal first ticks, DuplicateTimeTicks::Fail";
        std::vector<TradePriceOnly> update7
        {
            { 90, 0.90 },
            { 90, 0.91 },
            { 90, 0.92 },
        ...
        trades.insert(trades.begin(), update7.begin(), update7.end());
        trades[1].ticks_ = 91;
        trades[2].ticks_ = 92;
        verifyFetchAll(trades.data(), trades.size(), data, step, context);

        step = "prepend equal last ticks, DuplicateTimeTicks::Fail";
        std::vector<TradePriceOnly> update8
        {
            { 87, 0.87 },
            { 88, 0.88 },
            { 88, 0.89 }
        ...
        trades.insert(trades.begin(), update8.begin(), update8.end());
        trades[2].ticks_ = 89;
        verifyFetchAll(trades.data(), trades.size(), data, step, context);

        step = "append equal ticks, DuplicateTimeTicks::Fail";
        std::vector<TradePriceOnly> update9
        {
            { 151, 1.51 },
            { 151, 1.52 },
            { 151, 1.53 }
        ...
        trades.insert(trades.end(), update9.begin(), update9.end());
        trades[18].ticks_ = 152;
        trades[19].ticks_ = 153;
        verifyFetchAll(trades.data(), trades.size(), data, step, context);

        step = "append unequal ticks, DuplicateTimeTicks::Fail";
        std::vector<TradePriceOnly> update10
        {
            { 154, 1.54 },
            { 155, 1.55 },
            { 156, 1.56 }
        ...
        trades.insert(trades.end(), update10.begin(), update10.end());
        verifyFetchAll(trades.data(), trades.size(), data, step, context);

        step = "append equal first ticks, DuplicateTimeTicks::Fail";
        std::vector<TradePriceOnly> update11
        {
            { 157, 1.57 },
            { 157, 1.58 },
            { 159, 1.59 }
        ...
        trades.insert(trades.end(), update11.begin(), update11.end());
        trades[24].ticks_ = 158;
        verifyFetchAll(trades.data(), trades.size(), data, step, context);

        step = "append equal last ticks, DuplicateTimeTicks::Fail";
        std::vector<TradePriceOnly> update12
        {
            { 160, 1.60 },
            { 161, 1.61 },
            { 161, 1.62 }
        ...
        trades.insert(trades.end(), update12.begin(), update12.end());
        trades[28].ticks_ = 162;
        verifyFetchAll(trades.data(), trades.size(), data, step, context);

        step = "insert single block of unique ticks, DuplicateTimeTicks::Fail";
        std::vector<TradePriceOnly> update13
        {
            { 131, 1.31 },
            { 132, 1.32 },
            { 133, 1.33 }
        ...
        trades.insert(trades.begin() + 16, update13.begin(), update13.end());
        verifyFetchAll(trades.data(), trades.size(), data, step, context);

        step = "insert two blocks of unique ticks, DuplicateTimeTicks::Fail";
        std::vector<TradePriceOnly> update14
        {
            { 127, 1.27 },
            { 128, 1.28 },
            { 129, 1.29 },
        ...
        trades.insert(trades.begin() + 15, update14.begin(), update14.begin() + 3);
        trades.insert(trades.begin() + 22, update14.begin() + 3, update14.end());
        verifyFetchAll(trades.data(), trades.size(), data, step, context);

        step = "insert four blocks of both duplicate and unique ticks, DuplicateTimeTicks::Fail";
        std::vector<TradePriceOnly> update15
        {
            { 146, 1.46 },
            { 147, 1.47 },
            { 148, 1.48 },
        ...
        trades.insert(trades.begin() + 25, update15.begin(), update15.begin() + 4);
        verifyFetchAll(trades.data(), trades.size(), data, step, context);
    ...
    {
        const char* fileName{ "exploratory_trade_price-only_fetchTickRange_with_custom_maximumReadBufferBytes.h5" };
        remove(fileName);
        ...
        step = "create trade price-only data";
        std::shared_ptr<TradePriceOnlyData> data = instrument->openTradePriceOnlyData(true);
        ASSERT_NOT_NULL(data.get()) << step << ": instrument->openTradePriceOnlyData should return non-null data";
        ...
        std::vector<TradePriceOnly> range3
        {
            { 87, 0.87 },
            { 88, 0.88 },
            { 89, 0.89 },
        ...
        std::vector<TradePriceOnly> range4
        {
            { 146, 1.46 },
            { 147, 1.47 },
            { 148, 1.48 },
        ...
        verifyFetchTickRange(trades.data(), trades.size(), data, 50, 190, step, context);
        ...
        std::vector<TradePriceOnly> range5
        {
            { 136, 1.36 }
        };
