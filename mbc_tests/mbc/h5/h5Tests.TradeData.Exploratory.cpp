...
std::string to_string(const Trade& trade)
{
    ...
    oss << "t:" << trade.ticks_ << ", p:" << trade.price_ << ", v:" << trade.volume_;
    return oss.str();
}

TESTGROUP("h5 exploratory trade data")
{
    TESTCASE_WITH_CONTEXT("data count, first ticks, last ticks")
    {
        const char* fileName{ "exploratory_trade_dataCount_firstTicks_lastTicks.h5" };
        remove(fileName);
        ...
        step = "create trade data";
        std::shared_ptr<TradeData> data = instrument->openTradeData(true);
        ASSERT_NOT_NULL(data) << step << ": instrument->openTradeData should return non-null data";
    ...
    {
        const char* fileName{ "exploratory_trade_add_update_prepend_append_insert.h5" };
        remove(fileName);
    ...
    {
        const char* fileName{ "exploratory_trade_fetchTickRange_with_custom_maximumReadBufferBytes.h5" };
        remove(fileName);
        ...
        step = "create trade data";
        std::shared_ptr<TradeData> data = instrument->openTradeData(true);
        ASSERT_NOT_NULL(data.get()) << step << ": instrument->openTradeData should return non-null data";
