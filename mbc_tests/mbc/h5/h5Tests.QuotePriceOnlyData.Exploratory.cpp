...
std::string to_string(const QuotePriceOnly& quote)
{
    std::ostringstream oss;
    oss << "t:" << quote.ticks_ << ", ap:" << quote.askPrice_ << ", bp:" << quote.bidPrice_;
    return oss.str();
}

TESTGROUP("h5 exploratory quote price-only data")
{
    TESTCASE_WITH_CONTEXT("data count, first ticks, last ticks")
    {
        const char* fileName{ "exploratory_quote_price-only_dataCount_firstTicks_lastTicks.h5" };
        remove(fileName);
        ...
        step = "create quote price-only data";
        std::shared_ptr<QuotePriceOnlyData> data = instrument->openQuotePriceOnlyData(true);
        ASSERT_NOT_NULL(data) << step << ": instrument->openQuotePriceOnlyData should return non-null data";

        step = "empty data";
        verifyProperties(data, 0, 0, 0, step, context);

        step = "add initial data";
        std::vector<QuotePriceOnly> quotes
        {
            { 87, 0.87, 0.87 },
            { 88, 0.88, 0.88 },
            { 89, 0.89, 0.89 },
    ...
    {
        const char* fileName{ "exploratory_quote_price-only_add_update_prepend_append_insert.h5" };
        remove(fileName);
        ...
        std::vector<QuotePriceOnly> update1
        {
            { 100, 1.01, 1.01 }
        };
        ...
        std::vector<QuotePriceOnly> update2
        {
            { 100, 1.0, 1.0 },
            { 130, 1.31, 1.31 }
        };
        ...
        std::vector<QuotePriceOnly> update3
        {
            { 130, 1.3, 1.3 },
            { 150, 1.51, 1.51 }
        };
        ...
        std::vector<QuotePriceOnly> update4
        {
            { 150, 1.5, 1.5 }
        };
        ...
        std::vector<QuotePriceOnly> update5
        {
            { 97, 0.97, 0.97 },
            { 98, 0.98, 0.98 },
            { 99, 0.99, 0.99 }
        ...
        std::vector<QuotePriceOnly> update6
        {
            { 94, 0.94, 0.94 },
            { 94, 0.95, 0.95 },
            { 94, 0.96, 0.96 }
        ...
        std::vector<QuotePriceOnly> update7
        {
            { 90, 0.90, 0.90 },
            { 90, 0.91, 0.91 },
            { 90, 0.92, 0.92 },
        ...
        std::vector<QuotePriceOnly> update8
        {
            { 87, 0.87, 0.87 },
            { 88, 0.88, 0.88 },
            { 88, 0.89, 0.89 }
        ...
        std::vector<QuotePriceOnly> update9
        {
            { 151, 1.51, 1.51 },
            { 151, 1.52, 1.52 },
            { 151, 1.53, 1.53 }
        ...
        std::vector<QuotePriceOnly> update10
        {
            { 154, 1.54, 1.54 },
            { 155, 1.55, 1.55 },
            { 156, 1.56, 1.56 }
        ...
        std::vector<QuotePriceOnly> update11
        {
            { 157, 1.57, 1.57 },
            { 157, 1.58, 1.58 },
            { 159, 1.59, 1.59 }
        ...
        std::vector<QuotePriceOnly> update12
        {
            { 160, 1.60, 1.60 },
            { 161, 1.61, 1.61 },
            { 161, 1.62, 1.62 }
        ...
        std::vector<QuotePriceOnly> update13
        {
            { 131, 1.31, 1.31 },
            { 132, 1.32, 1.32 },
            { 133, 1.33, 1.33 }
        ...
        std::vector<QuotePriceOnly> update14
        {
            { 127, 1.27, 1.27 },
            { 128, 1.28, 1.28 },
            { 129, 1.29, 1.29 },
        ...
        std::vector<QuotePriceOnly> update15
        {
            { 146, 1.46, 1.46 },
            { 147, 1.47, 1.47 },
            { 148, 1.48, 1.48 },
    ...
    {
        const char* fileName{ "exploratory_quote_price-only_fetchTickRange_with_custom_maximumReadBufferBytes.h5" };
        remove(fileName);
        ...
        step = "create quote price-only data";
        std::shared_ptr<QuotePriceOnlyData> data = instrument->openQuotePriceOnlyData(true);
        ASSERT_NOT_NULL(data.get()) << step << ": instrument->openQuotePriceOnlyData should return non-null data";
        ...
        std::vector<QuotePriceOnly> range3
        {
            { 87, 0.87, 0.87 },
            { 88, 0.88, 0.88 },
            { 89, 0.89, 0.89 },
        ...
        std::vector<QuotePriceOnly> range4
        {
            { 146, 1.46, 1.46 },
            { 147, 1.47, 1.47 },
            { 148, 1.48, 1.48 },
        ...
        std::vector<QuotePriceOnly> range5
        {
            { 136, 1.36, 1.36 }
        };
