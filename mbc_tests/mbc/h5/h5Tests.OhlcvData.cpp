...
TESTGROUP("h5 ohlcv data")
{
    static const char* dataInstrument{ "/foo/bar" };

    static bool fetchAllData(std::vector<Ohlcv> & dataVector, const std::shared_ptr<OhlcvData> & data)
    {
        return data->fetchAll([&dataVector](const Ohlcv* ptr, unsigned long long cnt) noexcept
        {
            try
            {
                while (cnt-- > 0UL)
    ...
    {
        const char* fileName{ "ohlcv_data_maximum_read_buffer_bytes_has_correct_value_initially.h5" };
        remove(fileName);
        ...
        const std::shared_ptr<OhlcvData> data = instrument->openOhlcvData(OhlcvKind::Default, DataTimeFrame::Minute3, true);
    ...
    {
        const char* fileName{ "ohlcv_data_maximum_read_buffer_bytes_has_correct_value_when_set.h5" };
        remove(fileName);
    ...
    {
        const char* fileName{ "ohlcv_data_after_close_is_called_data_handle_is_unusable.h5" };
        remove(fileName);
    ...
    {
        const char* fileName{ "ohlcv_data_first_ticks_is_zero_when_data_is_empty.h5" };
        remove(fileName);
    ...
    TESTCASE("first ticks has correct value when data not empty")
    {
        ...
        const char* fileName{ "ohlcv_data_first_ticks_has_correct_value_when_data_not_empty.h5" };
        remove(fileName);
    ...
    {
        const char* fileName{ "ohlcv_data_last_ticks_is_zero_when_data_is_empty.h5" };
        remove(fileName);
    ...
    {
        const long ticksExpected = 42L;
        const char* fileName{ "ohlcv_data_last_ticks_has_correct_value_when_data_not_empty.h5" };
        remove(fileName);
    ...
    {
        const char* fileName{ "ohlcv_data_count_is_zero_when_data_is_empty.h5" };
        remove(fileName);
    ...
    {
        const char* fileName{ "ohlcv_data_count_ticks_has_correct_value_when_data_not_empty.h5" };
        remove(fileName);
    ...
    TESTCASE("spread duplicate time ticks does nothing when ticks are different")
    {
        ...
        const bool status = OhlcvData::spreadDuplicateTimeTicks(vec.data(), vec.size(), false);
    ...
    TESTCASE("spread duplicate time ticks does nothing when ticks are different (vector)")
    {
        const auto ticksFirst = 1001L, ticksLast = 1010L;
        std::vector<Ohlcv> vec{ {ticksFirst, 1.1, 1.1, 1.1, 1.1, 11}, {ticksLast, 1.2, 1.2, 1.2, 1.2, 12} };
        const bool status = OhlcvData::spreadDuplicateTimeTicks(vec, false);
    ...
    {
        const char* fileName{ "ohlcv_data_adds_different_ticks_when_duplicate_policy_is_fail.h5" };
        remove(fileName);
        const std::shared_ptr<File> file = File::openReadWrite(fileName, true);
        const std::shared_ptr<Instrument> instrument = file->openInstrument(dataInstrument, true);
        const std::shared_ptr<OhlcvData> data = instrument->openOhlcvData(OhlcvKind::Default, DataTimeFrame::Minute3, true);
    ...
    {
        const char* fileName{ "ohlcv_data_adds_different_ticks_when_duplicate_policy_is_skip.h5" };
        remove(fileName);
        const std::shared_ptr<File> file = File::openReadWrite(fileName, true);
        const std::shared_ptr<Instrument> instrument = file->openInstrument(dataInstrument, true);
        const std::shared_ptr<OhlcvData> data = instrument->openOhlcvData(OhlcvKind::Default, DataTimeFrame::Minute3, true);
    ...
    {
        const char* fileName{ "ohlcv_data_adds_different_ticks_when_duplicate_policy_is_update.h5" };
        remove(fileName);
        const std::shared_ptr<File> file = File::openReadWrite(fileName, true);
        const std::shared_ptr<Instrument> instrument = file->openInstrument(dataInstrument, true);
        const std::shared_ptr<OhlcvData> data = instrument->openOhlcvData(OhlcvKind::Default, DataTimeFrame::Minute3, true);
    ...
    {
        const char* fileName{ "ohlcv_data_does_not_add_all_equal_ticks_when_duplicate_policy_is_fail.h5" };
        remove(fileName);

        std::vector<Ohlcv> dataVector;
        const std::shared_ptr<File> file = File::openReadWrite(fileName, true);
        const std::shared_ptr<Instrument> instrument = file->openInstrument(dataInstrument, true);
        const std::shared_ptr<OhlcvData> data = instrument->openOhlcvData(OhlcvKind::Default, DataTimeFrame::Minute3, true);
    ...
    {
        const char* fileName{ "ohlcv_data_does_not_add_some_equal_ticks_when_duplicate_policy_is_fail.h5" };
        remove(fileName);
    ...
    {
        const char* fileName{ "ohlcv_data_add_skips_equal_ticks_when_duplicate_policy_is_skip.h5" };
        remove(fileName);
    ...
    {
        const char* fileName{ "ohlcv_data_add_updates_equal_ticks_when_duplicate_policy_is_update.h5" };
        remove(fileName);
    ...
    {
        const char* fileName{ "ohlcv_data_fetch_all_fetches_data_correctly.h5" };
        remove(fileName);
    ...
    {
        const char* fileName{ "ohlcv_data_fetches_tick_range_correctly_when_both_ticks_inside_range.h5" };
        remove(fileName);
        ...
        const bool statusFetch = data->fetchTickRange([&dataVector](const Ohlcv* ptr, unsigned long long cnt) noexcept
        {
            try
            {
                while (cnt-- > 0UL)
    ...
    {
        const char* fileName{ "ohlcv_data_fetches_tick_range_correctly_when_first_tick_outside_range.h5" };
        remove(fileName);
    ...
    {
        const char* fileName{ "ohlcv_data_fetches_tick_range_correctly_when_last_tick_outside_range.h5" };
        remove(fileName);
    ...
    {
        const char* fileName{ "ohlcv_data_fetches_tick_range_correctly_when_both_ticks_outside_range.h5" };
        remove(fileName);
    ...
    {
        const char* fileName{ "ohlcv_data_fetches_tick_range_correctly_when_both_ticks_after_last.h5" };
        remove(fileName);
    ...
    {
        const char* fileName{ "ohlcv_data_fetches_index_range_correctly_when_index_inside_and_count_inside_range.h5" };
        remove(fileName);
        ...
        const bool statusFetch = data->fetchIndexRange([&dataVector](const Ohlcv* ptr, unsigned long long cnt) noexcept
        {
            try
            {
                while (cnt-- > 0UL)
    ...
    {
        const char* fileName{ "ohlcv_data_fetches_index_range_correctly_when_index_zero_and_count_inside_range.h5" };
        remove(fileName);
    ...
    {
        const char* fileName{ "ohlcv_data_fetches_index_range_correctly_when_index_inside_and_count_outside_range.h5" };
        remove(fileName);
    ...
    {
        const char* fileName{ "ohlcv_data_fetches_index_range_correctly_when_index_zero_and_count_outside_range.h5" };
        remove(fileName);
    ...
    {
        const char* fileName{ "ohlcv_data_ticks_index_is_correct_when_tick_inside_and_matches.h5" };
        remove(fileName);
    ...
    {
        const char* fileName{ "ohlcv_data_ticks_index_is_correct_when_tick_inside_and_not_matches.h5" };
        remove(fileName);
    ...
    {
        const char* fileName{ "ohlcv_data_ticks_index_is_correct_when_tick_before_first.h5" };
        remove(fileName);
    ...
    {
        const char* fileName{ "ohlcv_data_ticks_index_is_correct_when_tick_after_last.h5" };
        remove(fileName);
    ...
    {
        const char* fileName{ "ohlcv_data_ticks_index_range_is_correct_when_both_ticks_inside_and_match.h5" };
        remove(fileName);
    ...
    {
        const char* fileName{ "ohlcv_data_ticks_index_range_is_correct_when_both_ticks_inside_and_not_match.h5" };
        remove(fileName);
    ...
    {
        const char* fileName{ "ohlcv_data_ticks_index_range_is_correct_when_first_tick_outside.h5" };
        remove(fileName);
    ...
    {
        const char* fileName{ "ohlcv_data_ticks_index_range_is_correct_when_last_tick_outside.h5" };
        remove(fileName);
    ...
    {
        const char* fileName{ "ohlcv_data_ticks_index_range_is_correct_when_both_between_two_ticks.h5" };
        remove(fileName);
    ...
    {
        const char* fileName{ "ohlcv_data_ticks_index_range_is_correct_when_both_ticks_outside.h5" };
        remove(fileName);
    ...
    {
        const char* fileName{ "ohlcv_data_delete_index_range_is_correct_when_both_indices_inside.h5" };
        remove(fileName);
    ...
    {
        const char* fileName{ "ohlcv_data_delete_index_range_is_correct_when_first_zero_last_inside.h5" };
        remove(fileName);
    ...
    {
        const char* fileName{ "ohlcv_data_delete_index_range_is_correct_when_first_inside_last_outside.h5" };
        remove(fileName);
    ...
    {
        const char* fileName{ "ohlcv_data_delete_index_range_is_correct_when_first_zero_last_outside.h5" };
        remove(fileName);
    ...
    {
        const char* fileName{ "ohlcv_data_delete_tick_range_is_correct_when_both_inside_and_match.h5" };
        remove(fileName);
    ...
    {
        const char* fileName{ "ohlcv_data_delete_tick_range_is_correct_when_both_inside_and_no_match.h5" };
        remove(fileName);
    ...
    {
        const char* fileName{ "ohlcv_data_delete_tick_range_is_correct_when_both_the_same_inside_and_match.h5" };
        remove(fileName);
    ...
    {
        const char* fileName{ "ohlcv_data_delete_tick_range_is_correct_when_both_the_same_inside_and_no_match.h5" };
        remove(fileName);
    ...
    {
        const char* fileName{ "ohlcv_data_delete_tick_range_is_correct_when_first_outside_last_inside_and_match.h5" };
        remove(fileName);
    ...
    {
        const char* fileName{ "ohlcv_data_delete_tick_range_is_correct_when_first_outside_last_inside_and_no_match.h5" };
        remove(fileName);
    ...
    {
        const char* fileName{ "ohlcv_data_delete_tick_range_is_correct_when_first_inside_last_outside_and_match.h5" };
        remove(fileName);
    ...
    {
        const char* fileName{ "ohlcv_data_delete_tick_range_is_correct_when_first_inside_last_outside_and_no_match.h5" };
        remove(fileName);
    ...
    {
        const char* fileName{ "ohlcv_data_delete_tick_range_is_correct_when_first_first_last_last.h5" };
        remove(fileName);
    ...
    {
        const char* fileName{ "ohlcv_data_delete_tick_range_is_correct_when_both_outside.h5" };
        remove(fileName);
    ...
    {
        const char* fileName{ "ohlcv_data_delete_all_before_tick_is_correct_when_tick_inside_and_matches.h5" };
        remove(fileName);
    ...
    {
        const char* fileName{ "ohlcv_data_delete_all_before_tick_is_correct_when_tick_inside_and_no_match.h5" };
        remove(fileName);
    ...
    {
        const char* fileName{ "ohlcv_data_delete_all_before_tick_is_correct_when_tick_before_first.h5" };
        remove(fileName);
    ...
    {
        const char* fileName{ "ohlcv_data_delete_all_before_tick_is_correct_when_tick_after_last.h5" };
        remove(fileName);
    ...
    {
        const char* fileName{ "ohlcv_data_delete_all_before_index_is_correct_when_index_inside.h5" };
        remove(fileName);
    ...
    {
        const char* fileName{ "ohlcv_data_delete_all_before_index_is_correct_when_index_zero.h5" };
        remove(fileName);
    ...
    {
        const char* fileName{ "ohlcv_data_delete_all_before_index_is_correct_when_index_outside.h5" };
        remove(fileName);
    ...
    {
        const char* fileName{ "ohlcv_data_delete_all_after_tick_is_correct_when_tick_inside_and_matches.h5" };
        remove(fileName);
    ...
    {
        const char* fileName{ "ohlcv_data_delete_all_after_tick_is_correct_when_tick_inside_and_no_matches.h5" };
        remove(fileName);
    ...
    {
        const char* fileName{ "ohlcv_data_delete_all_after_tick_is_correct_when_tick_before_first.h5" };
        remove(fileName);
    ...
    {
        const char* fileName{ "ohlcv_data_delete_all_after_tick_is_correct_when_tick_after_last.h5" };
        remove(fileName);
    ...
    {
        const char* fileName{ "ohlcv_data_delete_all_after_index_is_correct_when_index_inside.h5" };
        remove(fileName);
    ...
    {
        const char* fileName{ "ohlcv_data_delete_all_after_index_is_correct_when_index_zero.h5" };
        remove(fileName);
    ...
    {
        const char* fileName{ "ohlcv_data_delete_all_after_index_is_correct_when_index_outside.h5" };
        remove(fileName);
